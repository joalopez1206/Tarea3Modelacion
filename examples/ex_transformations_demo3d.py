...
if __name__ == "__main__":
    ...
    while not glfw.window_should_close(window):
        ...
        modelTransformPlane = tr.matmul([
                                    tr.rotationY(dx*100),
                                    tr.translate(-dx,0,0),
                                    tr.uniformScale(0.1),
                                    tr.rotationY(-np.pi/2)])
        modelTransformHelix = tr.matmul([
                                        tr.rotationY(dx*20),
                                        tr.translate(-dx,0,0),
                                        tr.uniformScale(0.1),
                                        tr.rotationY(-np.pi/2)])
        modelTransformPlane2 = tr.matmul([
                                    #tr.rotationX(dx*20),
                                    tr.translate(dx,0,0),
                                    tr.translate(-0.5,0,0.5),
                                    tr.uniformScale(0.1),
                                    tr.rotationY(-np.pi/2)])
        modelTransformHelix2 = tr.matmul([
                                        tr.translate(-0.5,0,0.5),
                                        tr.rotationX(dx*40),
                                        tr.translate(dx,0,0),
                                        tr.uniformScale(0.1),
        ...
        glUniformMatrix4fv(glGetUniformLocation(pipeline.shaderProgram, "model"), 1, GL_TRUE, modelTransformPlane2)
        pipeline.drawCall(gpuPlane2)

        glUniformMatrix4fv(glGetUniformLocation(pipeline.shaderProgram, "model"), 1, GL_TRUE,modelTransformHelix2)
        pipeline.drawCall(gpuHelix2)
